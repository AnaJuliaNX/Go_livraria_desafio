...
func main() {
	...
	//Rota para fazer o pedido
	router.HandleFunc("/usuarios/{usuario_id}/pedidos", biblioteca.FazerPedido).Methods(http.MethodPost)
	//Rota para selecionar os itens do pedido
	router.HandleFunc("/usuarios/{id}/pedidos/{pedido_id}/livros/{livro_id}", biblioteca.ItemDoPedido).Methods(http.MethodPost)
